...
posRoutes.post('/placeorder', function (req, res) {
            ...
            "ObjListItems": [
                {
                    "KISection": String,
                    "KIItemCode": Number,
                    "KIIsManualEntry": Boolean,
                    "KIIsComplimentry": Boolean,
                    "KIDiscountPer": Number,
                    "KIDiscountAmount": Number,
                    "KIDescription": String,
                    "KIPrintingDescription": String,
                    "KIToppingsSelected": String,
                    "KIQty": Number,
                    "KIRate": Number,
                    "KIBillSeries": String,
                    "KIRemark": String,
                    "KICourse": String,
                    "KISaleType": String,
                    "KIBillingSection": String,
                    "KIPreparedByKitchen": String,
                    "KIDepartment": String,
                    "KIUserCategory1": String,
                    "KIUserCategory2": String,
                    "KITaxCategory": String,
                    "KIReasonForComplimentary": String,
                    "KIReasonForDiscount": String,
                    "KIPreparationTime": Number
                }
            ]
        }
    };
    ...
    if(orderObj.drinks){
        if(orderObj.drinks.length>0){
            ...
            Pos.findOne({token: token}, function (err, adminFound) {
                if (!err && adminFound) {
                    Outlet.findOne({outletId:adminFound.outletId}, function (err, drinks) {
                        if(!err && drinks) {
                            Order.find({},{orderId:1}, function (err, orders) {
                                if (err) console.log(err);
                                    ...
                                    for (var i = 0; i < orderObj.drinks.length; i++) {
                                        for (var j = 0; j < drinks.drinks.length; j++) {
                                            response = true;
                                            if (orderObj.drinks[i].itemCode == drinks.drinks[j].itemCode) {
                                                console.log('drink matched');
                                                var drinkObj = {
                                                    userPrice: drinks.drinks[i].runningPrice.toString(),
                                                    categoryId: 0,
                                                    drinkType: orderObj.drinks[i].drinkType,
                                                    category: orderObj.drinks[i].drinkType,
                                                    name: orderObj.drinks[i].name,
                                                    basePrice: drinks.drinks[j].basePrice,
                                                    capPrice: drinks.drinks[j].capPrice,
                                                    runningPrice: drinks.drinks[j].runningPrice,
                                                    available: true,
                                                    demandLevel : 0,
                                                    demandRate : 0,
                                                    itemCode: orderObj.drinks[i].itemCode,
                                                    priceIncrementPerUnit: drinks.drinks[j].priceIncrementPerUnit,
                                                    numberofdrinks: orderObj.drinks[i].numberofdrinks,
                                                    regularPrice: drinks.drinks[j].regularPrice,
                                                    priceVariable: true,
                                                    remark: orderObj.drinks[i].remark,
                                                    description: ""
                                                };
                                                drink.push(drinkObj);
                                                order.drinks.push(drinkObj);
                                    ...
                                    if (i >= orderObj.drinks.length - 1 && j >= drinks.drinks.length - 1) {
                                        console.log('here');
                                        order.save(function (err, saved) {
                                            if (!err && saved) {
                                                res.json({
                                                    success: true,
                                                    data: {orderId: saved._id.toString()},
                                                    error: null
                                                });
                                            ...
                                            if (!err && outletFound != null) {
                                                ...
                                                if (g >= outletFound.drinks.length) {
                                                    ...
                                                    if (q >= orderObj.drinks.length) {
                                                        ...
                                                        drinks.forEach(function (eachOrderDrink, a) {
            ...
            if (!err && adminFound) {
                ...
                Order.find({},{orderId:1}, function (err, orders) {
                    if(!err && orders){
                        ...
                        order2.orderId = orders[0].orderId+1;
                        order2.outletId = adminFound.outletId;
                        order2.tableNumber = orderObj.tablenumber;
                        order2.status = 'confirmed';
                        order2.name = "POS - " + adminFound.userName;
                        order2.userId = 0;
                        order2.adminId = 0;
                        order2.orderType = "POS";
                        order2.orderDate = date;
                        order2.orderTime = time;
                        order2.confirmTime = time;
                        order2.created_at = newdate;
                        order2.updated_at = newdate;
                        order2.syncStatus = 1;
                        order2.foods = [];
                        order2.drinks = [];
                        ...
                        for (var i = 0; i < orderObj.foods.length; i++) {
                            var foodObj = {
                                foodId: orderObj.foods[i].itemCode,
                            ...
                            order2.foods.push(foodObj);
                        }
                        if (i >= orderObj.foods.length-1) {

                            order2.save(function (err, saved) {
                                if (!err && saved) {
                                    if(response == false){
                                        res.json({
                                            success: true,
                            ...
                            Outlet.findOneAndUpdate({
                                outletId: adminFound.outletId,
                                'tables.tableNumber': orderObj.tablenumber.toString()
                            }, {
                                $set: {'tables.$.status': 'occupied'}
                            }, {safe: true, new: true}, function (err, outletFound) {
                                if (!err && outletFound) {
    ...
    if(!orderObj.drinks && !orderObj.foods){
        res.json({
            success:false,
            data:null,
...
posRoutes.post('/bill', function (req, res) {
    ...
    console.log(billObj);
    var posBillObject = {
        SecretKey : billObj.SecretKey,
        UserName : billObj.UserName,
        Password : billObj.Password,
        SystemId : billObj.SystemId,
        PlatformId : billObj.PlatformId,
        TableNo : billObj.TableNo,
        Covers : billObj.Covers,
        TotalBaseAmount : billObj.TotalBaseAmount,
        TotalBillDiscount : billObj.UserName,
        TotalTax : billObj.TotalTax,
        TotalBillValue : billObj.TotalBillValue,
        OrderData : []
    };
    var tableNumber = billObj.TableNo;
    var bill = new Bill;
    var obj = {};
    var total = 0;
    var date1 = moment.utc().add(330, 'minutes');
                        ...
                        if(!err && billFound!=null){
                            ...
                            if (ordersMain.length > 0) {
                                ...
                                if (i1 >= ordersMain.length) {
                                    ...
                                    if (total > 0) {
                                        billFound.totalBill = parseInt(billObj.TotalBaseAmount);
                                        billFound.status = "unpaid";
                                    ...
                                    Bill.find({},{billId:1}, function (err, bills) {
                                        if (!err && bills) {
                                            ...
                                            if (total > 0) {
                                                ...
                                                bill.totalBill = parseInt(billObj.TotalBaseAmount);
                                                bill.status = "unpaid";
                                                bill.syncStatus = 1;
...
//API to cancel all orders and vacate table
posRoutes.get('/cancelorder', function (req, res) {
    var table = req.query.tableNumber;
    var billedOrderIds = [];
    var orderid;
    var outletid;
                                    ...
                                    Order.find({$and:[{outletId:outletid},{tableNumber:oldTable},{$or:[{status:'pending'},{status:'confirmed'},{status:'placed'},{orderId:{$in:billOrderId}}]}]}, function (err, orders) {
                                        if(!err && orders.length>0){
                                            for(var o=0;o<orders.length;o++){
                                                ...
                                                orders[o].tableNumber = newTable;
                                                orders[o].save(function (err) {
                                                    if(err) console.log(err);
                                                });
...
//API to change user table
posRoutes.get('/changetable', function (req, res) {
    var oldTableIn = req.query.oldTableNumber;
    var oldTable = oldTableIn.toString();
    var newTableIn = req.query.tableNumber;
    var newTable = newTableIn.toString();
...
//API to remove items from orders
posRoutes.get('/removeitems', function (req, res) {
...
//API to discount items
posRoutes.get('/discountitem', function (req, res) {
    var itemCode = req.body.itemCode;
    var oldPrice = req.body.oldPrice;
    var newPrice = req.body.newPrice;
    var type = req.body.itemType;
...
posRoutes.put('/billpaid', function (req, res) {
    ...
    Bill.findOneAndUpdate({outletId:posFound.outletId, tableNumber : tableno, status:'unpaid'}, {
        status: 'paid'
...
//API to make bill paid
posRoutes.get('/billpaid', function (req, res) {
    var logObj = {
        logTime: String,
        log: String
    };
...
//API to return item price
posRoutes.get('/itemprice', function (req, res) {
    var itemcode = req.query.ItemId;
    var token = req.headers['auth_token'];
    var success ={
        Status:1,
        itemsPriceList: [{
    ...
    Pos.findOne({token:token}, function (err, posFound) {
        if(!err && posFound){
            ...
            Outlet.findOne({outletId:posFound.outletId}, function (err, drinkList) {
                for (var d=0;d<drinkList.drinks.length;d++){
                    if(drinkList.drinks[d].itemCode == itemcode){
                        itemPrice = drinkList.drinks[d].runningPrice;
                    }
                }
                if(d>=drinkList.drinks.length){
                    res.json({
                        Status:1,
                        itemsPriceList: [{
                            ItemId:itemcode,
            ...
            res.json({
                success:false,
                data:null,
                error:'Error verifying Pos User, contact Database Admin'
            });
        }
    });
